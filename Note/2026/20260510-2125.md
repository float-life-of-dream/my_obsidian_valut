...
3. 插入（在第$i-1(1\leqslant i \leqslant n+1)$）个结点后插入一个值为X的新结点0
   ...
   5. 再找到链表的第$i-1$个结点，用$p$指向

   6. 然后修改指针，插入结点($p$之后插入的新结点是s)；
...
7.  删除（删除链表的第$i(1\le i \le n)$个位置上的结点）

   8. 先找到链表上第$i-1$个结点，用$p$指向；
   9. 在用指针s指向要被删除的结点（$p$的下一个结点）；
   10. 然后修改指针，删除$s$所至的结点；
   11. 最后释放所指结点的空间。
