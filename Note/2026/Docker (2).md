[跳转至](https://101.lug.ustc.edu.cn/Ch08/#docker)
...
将需要使用 Docker 的用户 [加入](https://101.lug.ustc.edu.cn/Ch05/#adduser) `docker` 用户组，以便使用命令行方式操作 Docker。 **注意： `docker` 用户组中的用户拥有与 root 等效的权限。**

### 配置 Registry Mirror（可选，推荐）

本节操作请参考其他文档

你可以参考 [https://yeasy.gitbook.io/docker\_practice/install/mirror](https://yeasy.gitbook.io/docker_practice/install/mirror) 获取目前可能可以使用的 registry mirror。
...
这里， `--rm` 代表容器停止运行（退出）之后，会被立刻删除； `--name` 参数代表给容器命名，如果没有加这个参数，那么 docker 会给容器随机起一个格式类似于 gracious\_brahmagupta 的名字。
...
这个例子尝试使用 Debian 仓库中的 RISC-V [交叉编译](https://101.lug.ustc.edu.cn/Ch07/supplement/#cross-compile-example) 工具链与 QEMU 模拟器构建一个简单的用于交叉编译的环境。
...
[跳转至](https://101.lug.ustc.edu.cn/Ch08/supplement/#supplement)
