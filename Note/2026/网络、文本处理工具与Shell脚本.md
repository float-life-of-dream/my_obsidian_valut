[跳转至](https://101.lug.ustc.edu.cn/Ch06/#shell)
...
![管道示例](https://101.lug.ustc.edu.cn/Ch06/images/pipe.png)
...
在 Windows 的 PowerShell 下使用 wget/curl
...
grep 事实上是非常强大的查找工具， [第九章](https://101.lug.ustc.edu.cn/Ch09/) 将在介绍正则表达式语法之后进一步介绍 grep。
...
sed 事实上是非常强大的文本操作工具，不仅支持正则表达式，而且能够做的操作也不止是替换。 [第九章](https://101.lug.ustc.edu.cn/Ch09/) 将进一步介绍 sed。
...
- 检查输入的第一个参j数 (设为 x) 是否存在。如果不是，输出提示，结束。
...
建议阅读 [第九章](https://101.lug.ustc.edu.cn/Ch09/) 后再尝试完成本题。
...
[跳转至](https://101.lug.ustc.edu.cn/Ch06/supplement/#supplement)
...
在 [第五章](https://101.lug.ustc.edu.cn/Ch05/) 中，我们介绍过 fork 炸弹。它通过创建大量进程消耗大量的系统资源，从而拖慢系统与正常进程的运行速度，增大响应时间，使得操作系统的正常运作受到较大影响。 [^1]
...
[跳转至](https://101.lug.ustc.edu.cn/Ch06/solution/#solution)
